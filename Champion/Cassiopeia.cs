...
namespace AIO7UP.Champions
{
    static class Cassiopeia
    {
        ...
        public static void OnGameLoad()
        {
            ...
            StartMenu = new Menu("Cassiopeia", "[7UP]Cassiopeia", true);
            ...
            ComboMenu.Add(new MenuBool("DisableAA", "Disable AA in Combo for faster Kite", true));
            ...
            QSet.Add(new MenuBool("UseQ2", "Try to hit =< 2 champions if can ?", false));
            QSet.Add(new MenuBool("UseQPok", "Use always [Q] if enemy is killable by Poison?", true));
            QSet.Add(new MenuBool("QComboDash", "Always use [Q] on Dash end position?", true));
            ComboMenu.Add(QSet);
            ...
            ClearMenu.Add(new MenuBool("UseQCL", "Use [Q] in clear ?", false));
            ClearMenu.Add(new MenuBool("UseWCL", "Use [W] in clear ?", false));
            ClearMenu.Add(new MenuBool("UseECL", "Use [E] in clear ?", false));
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            var Combo = Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo);
            var LastHit = Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LastHit);
            var LaneClear = Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.LaneClear);
            var Harass = Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Harass);
        ...
        private static void Combo()
        {
            ...
            if (ComboMenu["DisableAA"].GetValue<MenuBool>().Enabled && Orbwalker.ActiveMode.HasFlag(OrbwalkerMode.Combo))
            {
